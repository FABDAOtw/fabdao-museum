...
import { pageSize, exhibitionWorks, exhibitionDocuments, findExhibit } from './curation.js';
...
$('#app').innerHTML=`
  ...
  <div id="tour-ui" hidden>
    ...
    <section id="guide" aria-label="作品導覽" hidden></section><button id="restore-interface" hidden>顯示介面 · H</button><div id="resume-panel" hidden><span class="tiny">TAKE YOUR TIME</span><h2>停留，也是一種觀看。</h2><button id="resume" class="primary">繼續漫遊 <span>→</span></button><button data-open="catalogue" class="text-link">開啟館藏目錄</button></div>
  </div>
...
const pendingRooms=[false,false,false,false],hangingVersions=[0,0,0,0];
...
function collapseRoom(value=true){$('#room-card').hidden=value;$('#room-pill').setAttribute('aria-expanded',String(!value));}
...
function goToRoom(index:number){if(!museum||!museumReady){notify('3D 暫時無法使用，仍可瀏覽館藏。');return;}if(panel.open)closePanel();if(!entered)start();stopGuide();pause(false);museum.teleport(index);updateRoom(index);collapseRoom(false);}
...
async function hangRoom(index:number,page=pages[index]){if(!museum)return;const size=pageSize(index),version=++hangingVersions[index];pendingRooms[index]=true;try{await museum.displayRoom(index,roomWorks(index).slice(page*size,page*size+size),roomDocs(index));if(version===hangingVersions[index])pages[index]=page;}finally{if(version===hangingVersions[index])pendingRooms[index]=false;}}
async function faceItem(item:Artwork|DocumentItem,keepGuide=false){
  ...
  if(!target){notify('這件作品收錄於館藏目錄，目前未列入首展展牆。');return false;}
  const request=++focusRequest;
  try{
    if(pages[target.room]!==target.page||pendingRooms[target.room])await hangRoom(target.room,target.page);
  ...
  }catch{notify('這組作品尚未載入完成，原展牆保留。請稍後重試。');return false;}
}
...
function inspect(item:Artwork|DocumentItem,origin='catalogue',ids?:string[]){
  ...
  selectedItem=item;detailOrigin=origin;detailIds=ids||('artist' in item?roomWorks(exhibitions.findIndex(x=>x.theme===item.theme)).map(a=>a.id):documents.map(d=>d.id));
...
function renderPanel(name:string){
 ...
 if(name==='catalogue'){
  ...
  content.innerHTML=`<p class="eyebrow">FIND YOUR OWN PATH</p><h2 id="panel-title">展間地圖</h2><p class="panel-intro">四廳由拱廊串連。可直接抵達展間，或選一件作品開始正面觀看。</p><div class="floorplan">${exhibitions.map((x,i)=>`<button data-room="${i}" class="map-room ${entered&&room===i?'current':''}"><span class="map-number">0${i+1}</span><strong>${escapeHTML(x.title)}</strong><small>${escapeHTML(x.titleEn)}</small><span class="map-enter">${entered&&room===i?'● 你在這裡':'進入展間 →'}</span></button>`).join('')}</div><p class="footnote">入口 → 生成之間 → 群山成島 → 收藏作為行動 → 留白與記憶</p>${exhibitions.map((x,i)=>`<section class="map-exhibition"><h3>0${i+1} ${escapeHTML(x.title)}</h3><p class="panel-prose">${escapeHTML(x.question||x.subtitle)}</p><button class="text-link" data-guide-room="${i}">從本廳開始導覽 →</button>${artList(roomWorks(i))}</section>`).join('')}`;
  ...
  content.innerHTML=`<p class="eyebrow">MAKE YOURSELF COMFORTABLE</p><h2 id="panel-title">觀看設定</h2><p class="panel-intro">依照電腦效能選擇畫質。</p><label class="settings-label" for="quality">畫質</label><select id="quality"><option value="balanced">均衡 · 標準陰影</option><option value="high">細緻 · 提升解析度</option><option value="low">流暢 · 關閉即時陰影</option></select><p class="footnote">目前約 ${fps} FPS，為這台裝置的即時估計。移動不順時可選擇「流暢」。</p><h3>依自己的步調觀看</h3><p class="panel-prose">移動不加入頭部晃動。正面定位採直接抵達；亦可留在館藏目錄閱讀。聲音與互動原作需自行啟動，關閉作品會停止播放。</p><button id="setting-quiet" class="text-link">${quiet?'恢復介面':'專心觀看 · 暫時隱藏介面'}</button>`;
  const select=$<HTMLSelectElement>('#quality');select.value=museum?.quality||'balanced';select.onchange=()=>{museum?.setQuality(select.value);try{localStorage.setItem('fab-museum-quality',select.value);}catch{}};$('#setting-quiet').onclick=()=>{closePanel();setQuiet(!quiet);};
 }else if(name==='help'){
  content.innerHTML=`<p class="eyebrow">A SLOW WALK THROUGH ART</p><h2 id="panel-title">如何逛美術館</h2><div class="help-list">${[['W A S D','前後左右移動'],['拖曳滑鼠','環視空間；自由環視可鎖定游標'],['↑ ↓ ← →','前後移動、向左或向右看'],['Page Up / Page Down','向上或向下看'],['Enter / 點擊作品','閱讀瞄準的作品或文獻'],['慢看導覽','依策展順序，逐件正面抵達'],['H','隱藏或恢復介面'],['M / Esc','開啟地圖／釋放游標或暫停']].map(([key,desc])=>`<div><span>${key}</span><p>${desc}</p></div>`).join('')}</div><p class="footnote">讀作品時，方向鍵可平移放大的預覽。所有導覽按鈕可用 Tab 與 Enter 操作。</p>`;
 }
}
function renderCatalogue(){
 ...
 $('#catalogue-grid').innerHTML=entries.length?entries.map(a=>`<button class="collection-card" data-id="${escapeHTML(a.id)}"><span class="card-image">${'artist' in a&&a.image?`<img src="${escapeHTML(safeURL(a.image))}" alt="${escapeHTML(a.title)}" loading="lazy">`:`<span class="document-tile">${'artist' in a?'預覽尚未取得':'PUBLIC ARCHIVE'}</span>`}<span class="card-type">${'artist' in a?(a.featured?'首展選件':a.themeStatus==='metadata_review'?'資料已複核':'分類待複核'):'文獻'}</span></span><span class="card-caption"><span>${escapeHTML('artist' in a?a.artist:a.date)}</span><h3>${escapeHTML(a.title)}</h3><span class="card-arrow">↗</span></span></button>`).join(''):'<p class="empty-state">沒有符合的紀錄。可清除搜尋或調整篩選。</p>';
...
async function init(){
 try{
  ...
  museum.onStats=value=>{fps=value;};museum.onContextLost=()=>{museumReady=false;stopGuide();$('#enter').onclick=()=>showPanel('catalogue');$('#enter').textContent='開啟館藏目錄';showPanel('catalogue');notify('3D 顯示已中斷，可繼續使用館藏目錄。');};
  ...
  await Promise.all(exhibitions.map((_,i)=>hangRoom(i)));museumReady=true;if(panel.open)museum.setPaused(true);
