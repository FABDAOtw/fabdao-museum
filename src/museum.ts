  ...
  async displayRoom(index:number,artworks:Artwork[],documents:DocumentItem[]):Promise<void> {
    ...
    try {
      for(let i=0;i<pieces.length;i++) {
        ...
        if(!texture)this.label(this.text('預覽待補  ·  點擊閱讀來源','Preview pending · Select to read the source'),0,0,.25,0,w,Math.min(h,.4),'#665944','#e6dfce',frame);
        ...
        this.label([item.title,credit,...(hint?[hint]:[])].join('\n'),0,-h/2-(hint ? .22 : .245),.25,0,Math.max(w,2.45),hint ? .45 : .30,'#343a34','#dfd7c7',frame);
