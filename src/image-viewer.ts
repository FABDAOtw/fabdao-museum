...
export function mountImageViewer(container:HTMLElement,url:string,title:string){
  const abort=new AbortController(), signal=abort.signal;
  const stage=document.createElement('div');stage.className='image-stage';stage.tabIndex=0;stage.setAttribute('aria-label','作品預覽，可用加減按鈕縮放，放大後拖曳或方向鍵平移');
  ...
  const status=document.createElement('output');status.setAttribute('aria-live','polite');status.textContent='載入預覽';
  ...
  button('適合畫面',fit);button('−',()=>zoom(1/1.4)).setAttribute('aria-label','縮小作品');button('＋',()=>zoom(1.4)).setAttribute('aria-label','放大作品');button('100%',()=>{scale=1;paint();});toolbar.append(status);
  const full=button('全螢幕',()=>{if(document.fullscreenElement===container){void document.exitFullscreen();}else{void container.requestFullscreen?.().catch(()=>{status.textContent='此瀏覽器無法開啟全螢幕';});}});
  if(!document.fullscreenEnabled)full.hidden=true;
  container.classList.add('image-viewer');container.append(stage,toolbar);
  img.addEventListener('load',()=>{if(!disposed)fit();},{signal});img.addEventListener('error',()=>{status.textContent='預覽暫時無法載入，請使用作品來源';},{signal});img.src=url;
