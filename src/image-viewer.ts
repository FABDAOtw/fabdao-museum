...
export function mountImageViewer(container:HTMLElement,url:string,title:string,locale:'zh-TW'|'en'='zh-TW'){
  ...
  // Keep the viewer's flex layout on its own element. The detail surface is
  // also a flex container used to centre document covers; putting the
  // `image-viewer` class on that surface made the stage shrink to zero width
  // under the inherited `align-items: center`, leaving a blank preview.
  ...
  const full=button(t('全螢幕','Fullscreen'),()=>{if(document.fullscreenElement===viewer){void document.exitFullscreen();}else{void viewer.requestFullscreen?.().catch(()=>{status.textContent=t('此瀏覽器無法開啟全螢幕','Fullscreen is unavailable in this browser.');});}});
  if(!document.fullscreenEnabled)full.hidden=true;
  viewer.append(stage,toolbar);container.append(viewer);
  ...
  return {dispose(){disposed=true;abort.abort();resize.disconnect();if(document.fullscreenElement===viewer)void document.exitFullscreen();viewer.remove();}};
}
